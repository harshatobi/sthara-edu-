...
export default function GeneralAITutorPage() {
  const [messages, setMessages] = useState<{ sender: 'ai' | 'student', text: string }[]>([
    { sender: 'ai', text: 'Hi Aryan! I am your AI Tutor. You can ask me any general questions about your syllabus, homework, or concepts you want to understand better.' }
  ]);
  ...
  const handleSendMessage = async () => {
    ...
    const newMessages = [...messages, { sender: 'student' as const, text: inputMessage }];
    ...
    <div className="flex flex-col h-[calc(100vh-8rem)] animate-in fade-in duration-500 max-w-4xl mx-auto w-full">
      <div className="bg-white p-6 rounded-t-2xl shadow-sm border border-[#002147]/10 flex items-center justify-between z-10 relative">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h2 className="text-2xl font-bold text-[#002147]">General AI Tutor</h2>
            <p className="text-[#002147]/60 text-sm">Ask any doubt, anytime.</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto bg-[#f8fafc] border-x border-[#002147]/10 p-6 space-y-6">
        {messages.map((msg, idx) => (
          <div key={idx} className={`flex ${msg.sender === 'ai' ? 'justify-start' : 'justify-end'}`}>
            <div className={`flex max-w-[80%] items-start space-x-3 ${msg.sender === 'student' ? 'flex-row-reverse space-x-reverse' : ''}`}>
              
              <div className={`flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center ${msg.sender === 'ai' ? 'bg-[#dc143c]/10 text-[#dc143c]' : 'bg-[#002147] text-white'}`}>
        ...
        {isTyping && (
          <div className="flex justify-start">
            <div className="flex items-center space-x-3">
              <div className="flex-shrink-0 w-10 h-10 rounded-full bg-[#dc143c]/10 text-[#dc143c] flex items-center justify-center">
