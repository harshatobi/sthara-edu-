...
const USERS = [
  ...
  })),
  {
    ...
    teacherClass: '10A',
    subject: 'Mathematics',
    assignments: [{ class: '10A', subject: 'Mathematics' }]
  },
  {
    ...
    assignments: [{ class: '10A', subject: 'Social Studies' }]
  }
];
...
export default function SeedRealPage() {
      ...
      for (const u of USERS) {
        ...
        try {
          ...
          if (e.code === 'auth/email-already-in-use') {
            setStatus(`Error: ${u.email} already exists. Use the buttons below to fix data for existing users.`);
            return;
          } else { throw e; }
        }
        ...
        if (u.role === 'teacher') {
          ...
          userData.assignments = (u as any).assignments;
        }
  ...
  // Update classes for existing users — sets assignments array that dashboard reads
  const updateClasses = async () => {
    ...
    try {
      ...
      // Update both teachers — assignments array is what the dashboard reads
      const teachers = [
        { email: 'testteacher1@gmail.com', subject: 'Mathematics' },
        { email: 'testteacher2@gmail.com', subject: 'Social Studies' },
      ];
      for (const t of teachers) {
        setStatus(`Updating assignments for ${t.email}...`);
        ...
        if (!snap.empty) {
          await updateDoc(snap.docs[0].ref, {
            teacherClass: '10A',
            subject: t.subject,
            assignments: [{ class: '10A', subject: t.subject }]
          });
        }
      }

      setStatus('DONE! All students → Class 10A. Both teachers assigned to 10A. Log out and back in to see changes.');
    } catch (error: any) {
      console.error(error);
      setStatus(`Error: ${error.message}`);
    }
